...
const VideoResult: React.FC<VideoResultProps> = ({
  ...
  const [transformData, setTransformData] = useState({ rotateX: 0, rotateY: 0, hover: false });
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setTransformData({
      rotateX: y * -15, // Max 15 degree tilt
      rotateY: x * 15,
      hover: true,
    });
  };
  ...
  const handleMouseLeave = () => {
    setTransformData({ rotateX: 0, rotateY: 0, hover: false });
  };
  ...
  return (
    <div className="w-full relative flex flex-col items-center gap-8 p-12 bg-gray-800/50 rounded-lg border border-gray-700 shadow-2xl overflow-visible">
      ...
      <div
        ...
        style={{
          transform: `perspective(1000px) rotateX(${transformData.rotateX}deg) rotateY(${transformData.rotateY}deg) ${transformData.hover ? 'scale3d(1.02, 1.02, 1.02)' : 'scale3d(1, 1, 1)'}`,
          transition: transformData.hover ? 'transform 0.1s ease-out' : 'transform 0.5s ease-out',
        }}
        className={`w-full ${isPortrait ? 'max-w-xs aspect-[9/16]' : 'max-w-2xl aspect-video'
          } rounded-lg overflow-hidden bg-black shadow-[0_20px_50px_rgba(79,70,229,0.3)] border border-indigo-500/30 z-20`}
      >
        <video
          ref={videoRef}
          src={videoUrl}
      ...
      <div className="flex flex-wrap justify-center gap-4">
        ...
        {canExtend ? (
          ...
          <button
            ...
            title="Only 720p 16:9 videos can be extended">
            <SparklesIcon className="w-5 h-5" />
            Extend
          </button>
        )}
